...
public class AcaisTask extends AsyncTask<Acai, Void, JSONObject> {
    ...
    public AcaisTask(Context context, ActionType action, CallBackTask callBack) {
    ...
    @Override
    protected JSONObject doInBackground(Acai... acais) {
        switch (this.action) {
            case READ: return new AcaisWS().read();
            case READ_BY_ID: return new AcaisWS().readById(acais[0].getId());
            default: return null;
        }
    }
